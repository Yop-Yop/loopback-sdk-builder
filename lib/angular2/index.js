...
module.exports = function generate(ctx) {
   ...
   * Define import statement for the SDK NG Modules
   */
  function buildServiceDI(isIo, noOverride=false) {
    let dependencies = [`@Inject(HttpClient) protected ${noOverride ? '' : 'override '}http: HttpClient`];
    if (isIo === 'enabled') {
      dependencies.push(`@Inject(SocketConnection) protected ${noOverride ? '' : 'override '}connection: SocketConnection`);
    }
    dependencies = dependencies.concat([
      `@Inject(SDKModels) protected ${noOverride ? '' : 'override '}models: SDKModels`,
      `@Inject(LoopBackAuth) protected ${noOverride ? '' : 'override '}auth: LoopBackAuth`,
      `@Optional() @Inject(ErrorHandler) protected ${noOverride ? '' : 'override '}errorHandler: ErrorHandler`
    ])
  ...
  function buildMethodParams(model, methodName, params, isIo) {
    ...
    params.forEach((param, i, arr) => {
      ...
      } else if (param.type === 'Boolean' || param.type === 'boolean') {
        type = 'boolean | null';
      } else if (param.type === 'String' || param.type === 'string') {
        type = 'string | null';
      } else if (param.type === 'Integer' || param.type === 'integer' || param.type === 'Number' || param.type === 'number') {
        type = 'number | null';
      } else {
        type = param.type !== 'AccessToken' && param.type !== 'any' && param.type !== undefined
            ? capitalize(param.type) : 'any | null';
      }

      if (!type.match(/(^any$|LoopBackFilter|boolean|string|number)/) && availableClasses.indexOf(type) < 0) {
        type = 'any | null';
      }
      ...
      if (!param.required && (model.isUser && methodName === 'login') && param.arg === 'include') {
        ...
        value = param.default !== null && typeof(param.default) != 'undefined' ? ` = ${param.default}` : ` = null`;
        ...
        // Pour les types complexes, on ne peut pas utiliser "new type | null()" car c'est invalide
        // On utilise simplement null comme valeur par défaut pour les types union
        if (type.includes('|')) {
          value = !param.required ? ` = null` : '';
        } else {
